...
d = filter(all, Germline == F, Mutation_level >= 2)
...
#d4$bonferroni = p.adjust(d4$p_val, method = "bonferroni", n = nrow(d4)+45) #Cause we also test for the germline variatns
...
write.table(d4, "Data/Tmp/Somatic_goi.tsv", sep = "\t", col.names = T, row.names = F, quote  = F)
dout <<-d4

d4 = filter(d4, !is.na(sum_n_mutated) & sum_n_MSI)
d4 = mutate(d4, fill = ifelse(p_val < 0.05, "p<0.05", "p>0.05"))

write.table(d4, "Figures/FigX4/somatic_fig3.tsv", sep = "\t", col.names = T, row.names = F, quote = F)


# #Barplot
gB_Soma <<- ggplot(data = d4, aes(x = reorder(paste(Gene,": ", sum_n_MSI, " / ", sum_n_mutated, sep = ""), -sum_prop), y = sum_prop))+
  geom_histogram(stat = "identity", aes(fill = sign), position = "stack",
                 color = "black",width = 0.4, show.legend = T, size = 0.2)+
  geom_text(data = filter(d4, p_val < 0.01), aes(label = scales::scientific(p_val, digits = 2,prefix = " p \U2264 ")),
  ...
  ggtitle("Pathogenic somatic variants in DDR genes")+
  ylab("Proportion\nSamples with MSI")+
