...
pcawg = pcawg%>%
  ...
  filter(TYPE == "INS")
pcawg$Ins_seq = sub('.', '', pcawg$ALT) #The way its annotated in the vcf, we have the first base of the reference which is the same as the alt. We remove this to see the deleted seq
pcawg$Ins_length = nchar(pcawg$Ins_seq)
...
pcawg = pcawg%>%
  ...
  dplyr::filter(n_rep_first_char == Ins_length) #Only keep cases where all bases are he samee
...
write.table(pcawg, "Data/Del_ins_counts/Mononucleotide_repeat_insertions_06_01_22.tsv", sep ="\t", col.names = T, row.names = F)
