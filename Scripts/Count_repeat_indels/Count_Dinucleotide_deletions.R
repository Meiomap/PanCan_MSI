...
pcawg = pcawg%>%
  ...
  mutate(
    ...
    Del_seq = sub('.', '', REF), #The way its annotated in the vcf, we have the first base of the reference which is the same as the alt. We remove this to see the deleted seq
    del_length = nchar(Del_seq)
  )%>%
  filter(TYPE == "DEL", del_length == 2)
...
pcawg = pcawg%>%
  ...
  mutate(
    ...
    min_string = Del_seq #Copy this first base too 3 times, as this is the minimal string tto search for
  )%>%
  dplyr::filter(n_rep_first_char != del_length) #Only keep cases where bases are different
...
write.table(pcawg, "Data/Del_ins_counts/Dinucleotide_repeat_deletions_06_01_22.tsv", sep ="\t", col.names = T, row.names = F)
